...
async def check_models_ui(page, artifact, actor_name):
    ...
    for theme in ['light','dark']:
        for width in [390,1440]:
            ...
            await page.get_by_role('button',name='编辑 主模型',exact=True).click()
            await page.get_by_role('textbox',name='显示名称（选填）',exact=True).fill('保留的模型草稿')
            ...
            await expect(page.get_by_role('alert').filter(has_text='输入已保留')).to_be_visible()
            await expect(page.get_by_role('textbox',name='显示名称（选填）',exact=True)).to_have_value('保留的模型草稿')
            assert not await page.evaluate('document.documentElement.scrollWidth>innerWidth+1')
            ...
            await page.get_by_role('button',name='编辑 主模型',exact=True).click()
            await expect(page.get_by_role('textbox',name='显示名称（选填）',exact=True)).to_have_value('主模型')
            await page.get_by_role('textbox',name='显示名称（选填）',exact=True).press('Escape')
            await page.keyboard.press('Escape')
